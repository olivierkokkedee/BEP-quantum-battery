...
for iii in range(N_datapoints):
    ...
    # Find the index of the first peak in energy
    energy_peak_index = find_first_peak_index(E_t_num)
    
    # Find the index of the first peak in fluctuations
    fluc_peak_index = find_first_peak_index(fluc_t_num)

    # Store the corresponding time values for the first peaks
...
#Nb=2 and N=10, check small beats (also Nb=10 and N=20)
