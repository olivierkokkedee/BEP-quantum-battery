...
# Define your functions E_t and fluctuations here

def find_first_max_index(array, margin=0.05):
    max_value = np.amax(array)
    
    # Find indices where the value is within the margin of the maximum
    candidate_indices = np.where(np.isclose(array, max_value, rtol=margin))[0]

    # Return the first index from the candidates
...
for iii in range(N_datapoints):
    ...
    # Find the index of the true first maximum in energy
    energy_max_index = find_first_max_index(E_t_num)

    # Find the index of the true first maximum in fluctuations
    fluc_max_index = find_first_max_index(fluc_t_num)

    # Store the corresponding time values for the true first maxima
